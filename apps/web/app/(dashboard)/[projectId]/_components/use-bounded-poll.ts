...
/**
 * Mirrored from the domain detail page's own
 * `domains/_components/use-bounded-poll.ts` (itself mirrored from the
 * hosted verification page's `app/verify/[token]/_lib/use-bounded-poll.ts`)
 ...
 * itself is duplicated here rather than shared. Drives the "First run"
 * walkthrough's active auto-verify while its claimed sandbox domain is
 * `pending` (see `ClaimedDomainWalkthrough` in `onboarding-panel.tsx`):
