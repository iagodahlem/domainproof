...
/**
 * Escalating backoff, same shape as the api's own pending-recheck ladder
 * (`apps/api/src/modules/domains/domain/recheck-schedule.ts`) but tuned for
 * a human watching this page rather than a background worker: quick at
 * first (DNS often propagates in seconds against the sandbox, minutes
 * against real providers), settling at a steady 30s so a long-lived tab
 * doesn't hammer the api. The last entry repeats once exhausted.
 */
