  ...
  app/                              # ROUTING ONLY — every file here is a Next.js route file or that route's own private helper
    ...
    (dashboard)/                      # authenticated routes — route group, mounts the QueryProvider once
      ...
      [projectId]/                      # project routes live at the root (Vercel/Resend-style), not nested under /dashboard
        ...
        _components/                     #   overview-section.tsx, overview-skeleton.tsx, project-overview-view.tsx, empty-overview.tsx, overview-link-card.tsx
...
| A route's primary query (the data its main content needs to render)                                                    | A `*QueryOptions` factory in `apps/web/lib/query/<resource>.ts` (takes a `getToken` function, so the same options object works from both a server `auth().getToken` and a client `useAuth().getToken`) — the route's `page.tsx` calls `getQueryClient().prefetchQuery(...)` and wraps its client section in `<HydrationBoundary state={dehydrate(queryClient)}>`; that section's own component calls the matching `useSuspenseQuery` hook, never `lib/api` or `lib/query`'s raw `queryFn` directly |
