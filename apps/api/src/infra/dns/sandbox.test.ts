...
describe('sandboxJourneyFor', () => {
  it('parses each known journey from its plain label', () => {
    expect(sandboxJourneyFor('verified.test')).toEqual({
      ok: true,
      journey: 'verified',
    })
    ...
    expect(sandboxJourneyFor('wrong-value.test')).toEqual({
      ok: true,
      journey: 'wrong-value',
    })
    expect(sandboxJourneyFor('nxdomain.test')).toEqual({
      ok: true,
      journey: 'nxdomain',
    })
    expect(sandboxJourneyFor('flaky.test')).toEqual({
      ok: true,
      journey: 'flaky',
    })
    expect(sandboxJourneyFor('conflict.test')).toEqual({
      ok: true,
      journey: 'conflict',
    })
  })
  ...
  it('treats + suffixed domains sharing a journey as independent, both resolving', () => {
    expect(sandboxJourneyFor('verified+run1.test')).toEqual({
      ok: true,
      journey: 'verified',
    })
    expect(sandboxJourneyFor('verified+run2.test')).toEqual({
      ok: true,
      journey: 'verified',
    })
  })
  ...
  it('reports not_sandbox for a real-world domain', () => {
    expect(sandboxJourneyFor('example.com')).toEqual({
      ok: false,
      reason: 'not_sandbox',
    })
  })
...
describe('checkTxt over a sandbox resolver (full outcome taxonomy from elapsed time)', () => {
  it('verified: found immediately', async () => {
    ...
    expect(
      await checkTxt(resolver, challenge.recordHost, TOKEN, BRAND_SLUG),
