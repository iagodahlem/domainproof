...
describe('createNodeFetchFetcher', () => {
  it('returns the body and status on a plain success', async () => {
    const fetchImpl = async () =>
      new Response('domainproof-verify=abc', { status: 200 })
  ...
  it('reports timeout when the request is aborted', async () => {
    const fetcher = createNodeFetchFetcher({
      timeoutMs: 10,
    ...
    const fetchImpl = async (input: string) => {
      ...
      if (input === URL) {
        return new Response(null, {
          status: 302,
          headers: {
            location: 'https://example.com/.well-known/domainproof-challenge-2',
          },
        })
      }
  ...
  it('rejects a redirect to a different host', async () => {
    ...
    const fetchImpl = async (input: string) => {
      ...
      return new Response(null, {
        status: 302,
        headers: {
          location: 'https://evil.example/.well-known/domainproof-challenge',
        },
      })
    }
    const fetcher = createNodeFetchFetcher({ fetchImpl })
  ...
  it('rejects a redirect to a plain-http Location', async () => {
    ...
    const fetchImpl = async (input: string) => {
      ...
      return new Response(null, {
        status: 302,
        headers: {
          location: 'http://example.com/.well-known/domainproof-challenge',
        },
      })
    }
    const fetcher = createNodeFetchFetcher({ fetchImpl })
