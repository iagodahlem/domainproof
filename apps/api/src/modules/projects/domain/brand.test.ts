...
describe('validateBrandSlug', () => {
  it('accepts the default brand slug', () => {
    expect(validateBrandSlug(DEFAULT_BRAND_SLUG)).toEqual({
      ok: true,
  ...
  it('accepts a slug with an internal hyphen', () => {
    expect(validateBrandSlug('sky-lane')).toEqual({
      ok: true,
      slug: 'sky-lane',
    })
  })
  ...
  it('trims surrounding whitespace before validating', () => {
    expect(validateBrandSlug('  skylane  ')).toEqual({
      ok: true,
      slug: 'skylane',
    })
  })
  ...
  it('rejects a 1-character slug', () => {
    expect(validateBrandSlug('a')).toEqual({
      ok: false,
  ...
  it('rejects a 33-character slug', () => {
    ...
    expect(validateBrandSlug(slug)).toEqual({
      ok: false,
      reason: 'invalid_format',
    })
  })
  ...
  it('rejects an empty string', () => {
    expect(validateBrandSlug('')).toEqual({
      ok: false,
      reason: 'invalid_format',
    })
  })
  ...
  it('rejects a slug with an underscore', () => {
    expect(validateBrandSlug('sky_lane')).toEqual({
      ok: false,
      reason: 'invalid_format',
    })
  })
  ...
  for (const reserved of RESERVED_BRAND_SLUGS) {
    it(`rejects the reserved slug "${reserved}"`, () => {
      expect(validateBrandSlug(reserved)).toEqual({
        ok: false,
        reason: 'reserved',
      })
    })
