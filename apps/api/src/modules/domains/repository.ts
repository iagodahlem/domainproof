...
export interface ClaimInsert {
  ...
  mode: DomainMode
  domain: string
  /** Set only at claim time — see `infra/db/schema.ts`'s `externalId` doc comment. `undefined`/omitted stores `null`. */
  externalId?: string
  ...
  nextCheckAt: Date
  /** The Frontend API plane's per-claim bearer credential — see `infra/db/schema.ts`'s `frontendToken` doc comment. Generated by the caller via core's `generateToken()`. */
...
export interface ListByProjectPaginatedOptions {
  ...
  externalId?: string
  /** Exact match against `domains.domain` (expects an already-normalized hostname — see `service.ts`'s `listDomains`). */
  domain?: string
}
...
export interface VerificationAttemptInsert {
  ...
  /**
   * The check this attempt actually ran — `undefined` leaves
   * `last_check_result` untouched (used by `expireOverdueGraceWindows`,
   * which persists a status transition without ever running a check; see
   * `infra/db/schema.ts`'s doc comment). Always provided by `verifyRow`'s
   * two branches (the expiry short-circuit and the real DNS-check path),
   * since both always have a definitive outcome to record.
   */
  lastCheckResult?: {
    outcome: string
...
export interface DomainsRepository {
  ...
  /**
   * A project's domains, newest first, cursor-paginated on
   * `(created_at, id)`, optionally narrowed by `mode`/`externalId`/`domain`
   * (see `ListByProjectPaginatedOptions`). Serves both planes: the
   * dashboard's cross-mode domains table (no `mode` filter — `mode` is a
   * per-row field there, not something to filter by) and v1's mode-scoped,
   * optionally-filtered list. Fetches `limit + 1` rows to decide `hasMore`
  ...
  /**
   * Looks up a domain by its Frontend API `frontend_token` — the token
   * itself is the entire authorization (it identifies exactly one domain
   * claim), so this needs no project/mode scoping, unlike every other
   * finder here. Returns `undefined` for an unknown token, indistinguishable
   * from any other lookup miss — see `infra/db/schema.ts`'s `frontendToken`
   * doc comment.
   */
  findByFrontendToken(token: string): Promise<DomainRow | undefined>
...
export function createDomainsRepository(db: Database): DomainsRepository {
  return {
    ...
    async listByProjectPaginated(
      ...
      if (mode) {
        conditions.push(eq(domains.mode, mode))
      }
      if (externalId !== undefined) {
        conditions.push(eq(domains.externalId, externalId))
      }
      if (domain !== undefined) {
        conditions.push(eq(domains.domain, domain))
      }
      if (cursorCondition) {
    ...
    async findByFrontendToken(token) {
      const [row] = await db
        .select()
        .from(domains)
        .where(eq(domains.frontendToken, token))
        .limit(1)
      return row
    },
      ...
      const [domainRow] = await db
        ...
        .set({
          ...
          ...(values.lastCheckResult !== undefined
            ? { lastCheckResult: values.lastCheckResult }
            : {}),
        })
