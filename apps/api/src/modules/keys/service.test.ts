...
describe('createKey', () => {
  it('returns a parseable key and a display-safe row with no secret material', async () => {
    ...
    expect(result.apiKey.maskedKey).not.toContain(
      parsed.ok ? parsed.value.secret : '',
    )
...
describe('revokeKey', () => {
  ...
  it("returns null for a key id that doesn't belong to the project", async () => {
    ...
    expect(
      await service.revokeKey('project_b', created.apiKey.keyId),
    ).toBeNull()
  })
...
describe('rotateKey', () => {
  ...
  it("returns null for a key id that doesn't belong to the project", async () => {
    ...
    expect(
      await service.rotateKey('project_b', created.apiKey.keyId),
    ).toBeNull()
  })
})
