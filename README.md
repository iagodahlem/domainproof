...
| `VERIFICATION_BASE_URL`                                                      | No        | Base URL (including the `/verify` path) the public API's `verificationUrl` and the Cloudflare one-click callback's redirect are built against. Unset defaults to `https://domainproof.dev/verify` when `NODE_ENV` is `production`, and to `http://localhost:3000/verify` in `development`/`test`; set explicitly for staging/preview to point at that environment's own web app instead. |
...
| `DOMAINPROOF_BASE_URL`                                                       | No        | Where the hosted MCP endpoint's per-request SDK client sends tool-call requests — this same service's own `/v1` plane. Unset defaults to the SDK's own production default (`https://api.domainproof.dev`) when `NODE_ENV` is `production`, and to `http://localhost:$PORT` in `development`/`test`; override explicitly for staging.                                                     |
...
| `DEMO_DOMAINPROOF_BASE_URL`                             | No          | Overrides the SDK's production default for the `/demo` app. Unset already defaults to `http://localhost:3001` under `next dev` (`NODE_ENV=development`); set explicitly for staging/preview, where `NODE_ENV` is `production`.                                                              |
