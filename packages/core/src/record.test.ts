...
describe("challengeHost", () => {
  it("returns the TXT challenge host for a plain domain", () => {
    expect(challengeHost("example.com", BRAND_SLUG)).toBe("_domainproof-challenge.example.com");
  });

  it("returns the TXT challenge host rooted at the registrable domain", () => {
    expect(challengeHost("sub.acme.co.uk", BRAND_SLUG)).toBe("_domainproof-challenge.acme.co.uk");
  });
...
describe("recordValue / parseRecordValue", () => {
  ...
  it("tolerates surrounding whitespace", () => {
    const token = generateToken();
    const result = parseRecordValue(`  ${recordValue(token, BRAND_SLUG)}  \n`, BRAND_SLUG);
  ...
  it("tolerates enclosing double quotes", () => {
    const token = generateToken();
    const result = parseRecordValue(`"${recordValue(token, BRAND_SLUG)}"`, BRAND_SLUG);
  ...
  it("rejects values missing the prefix", () => {
    expect(parseRecordValue("some-other-value=abc", BRAND_SLUG)).toEqual({ ok: false });
  });

  it("rejects a case-mismatched prefix", () => {
    expect(parseRecordValue("Domainproof-Verify=abc", BRAND_SLUG)).toEqual({ ok: false });
  });

  it("rejects a bare prefix with no token", () => {
    expect(parseRecordValue(recordValuePrefix(BRAND_SLUG), BRAND_SLUG)).toEqual({ ok: false });
  });
