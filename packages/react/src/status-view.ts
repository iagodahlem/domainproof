import type { DomainStatus, VerificationCheck } from './types'

export type StatusTone = 'pending' | 'success' | 'warning' | 'danger'

/**
 * A pure, presentation-only view of one (status, check) pair — everything
 * `VerificationView`'s `outcome` prop needs, with no React and no
 * fetching, so the full state matrix (5 statuses × 5 check outcomes, most
 * combinations unreachable in practice but still possible over the wire)
 * is unit-testable without rendering anything. See `describeStatus`'s doc
 * comment for the matrix this is derived from. Mirrors the hosted
 * verification page's own `describeStatus` (`apps/web/app/verify/[token]/_lib/status-view.ts`)
 * — duplicated rather than imported for the same reason `./types` duplicates
 * the wire shapes: this package ships to consumers who never have that
 * workspace installed.
 */
...
/**
 * Every outcome `VerificationCheck.outcome` can carry. The wire type
 * widens this to `string`, so this is an `if`/`else if` chain with an
 * honest fallback branch for an outcome string this package doesn't
 * recognize, not a compiler-checked exhaustive switch.
 */
...
/**
 * The state matrix this widget renders:
 *
 * - `pending` + `wrong_value` is a *hard* failure (a syntactically valid
 *   but wrong record is actionable information, not "not there yet"), so
 *   it transitions straight to `failed`; it can never be observed as
 *   `(pending, wrong_value)` here.
 ...
 * - `failed` has no self-service recovery this package can offer — a
 ...
 *   crashing — the API contract could change in ways this package's tests
